...
Cypress.Commands.add('drush', (command, args = [], options = {}) => {
  let drushCommand = `${Cypress.env('drushCommand')} ${command} ${stringifyArguments(args)} ${stringifyOptions(options)} -y`;
  let drushOptions = { failOnNonZeroExit: false };
  cy.log(`drush command: ${drushCommand}`)
  return cy.exec(drushCommand, drushOptions)
});
...
/**
 * Make a payment via Windcave (Commerce DPS).
 */
Cypress.Commands.add('windcavePayment', () => {
  // Payment via DPS.
